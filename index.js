...
// =========================
// Por padrão salva em ./data/database.json.
// No Railway, crie um Volume montado em /app/data para não perder planos/produtos ao atualizar o index.js.
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("addpainelproduto")
    .setDescription("Cria um painel com menu de produtos da loja.")
    .addStringOption(option =>
      option
        .setName("titulo")
        .setDescription("Título do painel de produtos")
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName("descricao")
        .setDescription("Descrição do painel de produtos")
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName("imagem")
        .setDescription("Link da imagem do painel")
        .setRequired(false)
    )
    .addStringOption(option =>
      option
...
client.on("interactionCreate", async interaction => {
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === "addpainelproduto") return handleAddPainelProduto(interaction);
    ...
    if (interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === "product_menu") return handleProductMenuSelect(interaction);
    }
...
async function handleAddPainelProduto(interaction) {
  if (!isAdmin(interaction)) {
    return interaction.reply({
      content: "❌ Apenas administradores podem criar painel de produtos.",
      ephemeral: true
    });
  }

  const products = getGuildProducts(interaction.guild.id).filter(produto => Number(produto.estoque ?? 0) > 0);

  if (!products.length) {
    return interaction.reply({
      content: "❌ Nenhum produto com estoque foi encontrado. Cadastre produtos usando /addproduto antes de criar o painel.",
      ephemeral: true
    });
  }
  ...
  const imagem = interaction.options.getString("imagem") || null;
  const cor = interaction.options.getString("cor") || "#FFFFFF";
  const rodape = interaction.options.getString("rodape") || "Star Applications • Menu de Produtos";
  ...
  const listedProducts = products.slice(0, 25);

  const embed = new EmbedBuilder()
    .setTitle(titulo)
    .setDescription(
      `${descricao}\n\n` +
      `Selecione um produto no menu abaixo para abrir um carrinho privado.\n\n` +
      listedProducts
        .map((produto, index) => {
          return `**${index + 1}.** ${produto.nome || "Produto"} — ${moneyBR(produto.preco)} • Estoque: ${produto.estoque ?? 0}`;
        })
        .join("\n")
    )
    .setColor(cor)
  ...
  const menu = new StringSelectMenuBuilder()
    ...
    .addOptions(
      listedProducts.map(produto => ({
        label: String(produto.nome || "Produto").slice(0, 100),
        description: `${moneyBR(produto.preco)} • Estoque: ${produto.estoque ?? 0}`.slice(0, 100),
  ...
  return interaction.reply({
    content: `✅ Painel de produtos criado com ${listedProducts.length} produto(s).`,
    ephemeral: true
  });
}
...
async function handleComprarProduto(interaction) {
  ...
  return createProductCart(interaction, productId);
}

async function handleProductMenuSelect(interaction) {
  const productId = interaction.values?.[0];

  if (!productId) {
    return interaction.reply({
      content: "❌ Produto inválido. Tente selecionar novamente.",
      ephemeral: true
    });
  }
...
async function createProductCart(interaction, productId) {
  const products = getGuildProducts(interaction.guild.id);
